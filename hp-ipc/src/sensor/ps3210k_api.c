...
{
	{10, 100,64,  12,  0,0,   20,22,19,  32,40,16,1},//gain:0-7
	{5,  100,64,  12,  0,0,    25,22,20,  32,40,16,1},//gain:8-
	{0,  110,64,  12,  0,0,   35,22,20,  32,40,16,1},//gain:16-  
	{0,  115,64,  12,  0,0,   35,24,23,  32,40,16,1},//gain:32-
	{0,  140,64,  12,  0,0,   45,28,23,  32,40,16,1},//gain:64-
	{0,  140,64,  12,  0,0,   50,26,24,  32,40,16,1},//gain:128-
};

const HI_U8 au8VarStrength[][16] = 
{
	/*1        2         4        8        16      32       64    128*/
	{160,  160,  160,  160,  160,  178,  195, 180,  180,  150,150,150,150,150,150,150,150},
...
{
	/*1 	  2    4     8 	 16	 32	  64	 128*/
	{0,  0,  0,  0, 15, 13, 16,  20,  20,  20,20,20,20,20,20,20},
...
{
	/*1 	  2 	  4	  8 	 16   32	64   128*/
	{0,  0,  0,  0,  5, 6,	7,  7,	7,  6,	6,  6,	6,  6,	6,  6},
...
{
	/*1 	    2 	4	  8 	    16    32	64   128*/
	{1500,1500,1500,1500,1500,1375,1375,1375,1375,1375,1375,1375,1375,1375,1375,1375},
};
/*== NR/3DNR end ==*/
...
{
	/*1      2       4      8      16     32     64    128*/
	{140, 130, 100, 100, 100,   74,  45,  40,  30,45,45,45,45,45,45,45},
}; 

const HI_U8 au8SharpenUd[][16] = 
{
	/*1      2       4      8      16     32     64    128*/
	{115, 110,  85,  80,  80,  64,  35,  35,  28,35,35,35,35,35,35,35},
};

HI_U8 au8OverShoot[][16] = 
{
	/*1      2       4      8      16     32     64    128*/
	{70,  68,  60,  60,  60,  32,  26,  24,  20,26,26,26,26,26,26,26},
};

const HI_U8 au8UnderShoot[][16] = 
{
	/*1      2       4      8      16     32     64    128*/
	{107, 100,  95,	95,  95,  56,  35,	33,  30,35,35,35,35,35,35,35},
};

const HI_U8 au8TextureNoiseThd[][16] =
{
	/*1 	  2 	 4	8    16    32	64   128*/
	{0,  0,  0,	0,	0,	8,	13,	15,	25,13,13,13,13,13,13,13},
};
...
const ISP_DRC_ATTR_S stDRC[]=
	...
	{1,  10,0,  2,192,60,2,  0,48,0,  OP_TYPE_MANUAL,  {30,0,32},  {128,54,32},},
	{1,  10,0,  2,192,60,2,  0,48,0,  OP_TYPE_MANUAL,  {20,0,32},  {128,54,32},},
	{1,  10,0,  2,192,60,2,  0,48,0,  OP_TYPE_MANUAL,  {10,0,32},  {128,54,32},},
	{1,  10,0,  2,192,60,2,  0,48,0,  OP_TYPE_MANUAL,  {00,0,32},  {128,54,32},},
	{1,  10,0,  2,192,60,2,  0,48,0,  OP_TYPE_MANUAL,  {00,0,32},  {128,54,32},},
...
{
	/*1 	     2 	 4	  8    16      32    64    128*/
	{135, 128, 100, 99,  99,  90,  80,  70,56,56,56,56,56,56,56,56},
	{110, 110, 110, 110, 100, 90,  90,  60,56,56,56,56,56,56,56,56},
	{160, 160, 160, 160, 160, 150, 130, 110,56,56,56,56,56,56,56,56},
};	
/*== Saturation end ==*/
...
void PS3210k_config(void)
{
	...
	stExpAttr.stAuto.stExpTimeRange.u32Max = 80000;
	stExpAttr.stAuto.stAGainRange.u32Max = 65536;	
	stExpAttr.stAuto.stDGainRange.u32Max = 1024;
	stExpAttr.stAuto.stISPDGainRange.u32Max = 2096;
	stExpAttr.stAuto.enAEMode = AE_MODE_FIX_FRAME_RATE;
	...
	while(1)
	{
		...
		{	
			...
			if(public_get_scene() == SCENE_DAY)
			{
				...
				{
					PS3210k_SetExposure(1);
					PS3210k_SetCCM(1);
					PS3210k_SetSaturation(1);	
					PS3210k_SetDCI(1);
				}
				else
				{
					PS3210k_SetExposure(2);
					PS3210k_SetCCM(2);
					PS3210k_SetSaturation(2);
					PS3210k_SetGamma(1);
					PS3210k_SetDCI(1);
				}
			...
			else
			{
				...
				ISP_EXPOSURE_ATTR_S stExpAttr;
				HI_MPI_ISP_GetExposureAttr(IspDev, &stExpAttr);
				stExpAttr.stAuto.u8Compensation = 55;
				HI_MPI_ISP_SetExposureAttr(IspDev,&stExpAttr);
				PS3210k_SetCCM(2);
				PS3210k_SetSaturation(2);
				PS3210k_SetDCI(1);
...
int PS3210k_GetDenoiseLevel(int intTimes, float gainTimes)
{
	...
	else if(gainTimes < 35.0)
	{
		level_t = 3;
	}
	else if(gainTimes < 75.0)
	{
		level_t = 4;
	}
	else
